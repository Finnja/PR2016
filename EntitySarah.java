                    ...
                    // UDP MESSAGE (pour faire le tour de l'anneau)
                        ...
                        // message est bon
                            ...
                            // voir si le message a déjà fait le tour de l'anneau
                                ...
                                if((mess_mots[0]).equals("APPL") && (mess_mots[2]).equals("TRANS###") && (mess_mots[3]).equals("REQ")){
                                	...
                                	if (findFile(fName, currentDir)){
                                		...
                                		//System.out.println( "FILE SIZE IN INT BYTES ==========> " + len);
                                		...
                                		while((bytes_read < len) && (n != -1)){        
                                			...
                                			if (left >= poss_size){
                                				...
                                				paquet_send = new DatagramPacket(udp_data, 
                                						udp_data.length, ia);
                                				System.out.println("En train d'envoyer... greater " + message); 
                                				System.out.println("greater length " + udp_data.length);
                                				dso.send(paquet_send);
                                				...
                                				paquet_send = new DatagramPacket(udp_data, 
                                						udp_data.length, ia);
                                				System.out.println("En train d'envoyer... smaller " + message); 
                                				System.out.println("smaller length " + udp_data.length);
                                				dso.send(paquet_send);
                                			}
                                			no_mess += 1;
                                		}                                	 
                                ...
                                //NEW CODE SEN
                                if((mess_mots[0]).equals("APPL") && (mess_mots[2]).equals("TRANS###") && (mess_mots[3]).equals("SEN")){
                                	System.out.println("-----------> MESSAGE WITH SEND");
                                	String trans_id = mess_mots[4].trim();
                                	String currentDir = System.getProperty("user.dir");
                                	String path = currentDir.trim() + "/" + fNamee.trim();
                        			...
                        			FileOutputStream outputStream = new FileOutputStream(fNamee);
                        			String putInFile = "";
                        			ByteBuffer buff1;
                                	for (int i=0; i<nummesss; i++){
                                		// recoit le message
                                		//buff1 = ByteBuffer.allocate(1024);
                                        dc_ec.receive(buff);
                                        String messageSEN = new String(buff.array(), 0, buff.array().length);
                                        //System.out.println("BUFFER ARRAY PRINTING ---> " +new String (buff.array()));
                                        buff.clear();
                                        
                                        //InetSocketAddress ia = new InetSocketAddress(this.adr_suiv, this.port_suiv);
                                        System.out.println("Recu SENT: " + messageSEN);
                                        
                                        //String[] mess_mots1 = messageSEN.split(" ");
                                        //String encCont = mess_mots1[7];
                                        String[] mess_mots1 = messageSEN.split(" ");
                                        int n = mess_mots1.length-7;
                                        System.out.println("int number length array " + n);
                                        String[] mess_contenu = new String[n];
                                        System.arraycopy(mess_mots1, 7, mess_contenu, 0, n);
                                        //String[] mess_contenu = Arrays.copyOfRange(mess_mots1, 7, mess_mots1.length);
                                        String contenu = Arrays.toString(mess_contenu);
                                        //String contenu = "poop poop ";
                                        putInFile = putInFile + contenu; 
                                        
                                        byte[] encBuf = putInFile.getBytes();    
                                        //System.out.println("BUFFER ARRAY TRYING: ----> " + Arrays.toString(encBuf));
                                ...
                                if((mess_mots[0]).equals("APPL") && (mess_mots[2]).equals("TRANS###") && (mess_mots[3]).equals("ROK")){
                                	System.out.println("-----------> MESSAGE WITH ROK FIRST");
                          ...
                          //NEW CODE
                            ...
                            /*
                            if((mess_mots[0]).equals("APPL") && (mess_mots[2]).equals("TRANS###") && (mess_mots[3]).equals("ROK")){
                            	System.out.println("-----------> MESSAGE WITH ROK SECOND");
                            	String fName = mess_mots[6].trim();
                            	int nummess = Integer.parseInt(mess_mots[7].trim());
                            	String trans_id = mess_mots[4].trim();
                            	String currentDir = System.getProperty("user.dir");
                            	...
                            	for (int i=0; i<nummess; i++){
                            		...
                            		buff1 = ByteBuffer.allocate(1024);
                                    dc_ec.receive(buff1);
                                    String messageSEN = new String(buff1.array(), 0, buff1.array().length);
                                    //System.out.println("BUFFER ARRAY PRINTING ---> " +new String (buff.array()));
                                    buff1.clear();
